...
f = open('result/scienceQA_sccot_all_test_Qwen_1_5_32B_T0.9_p0.7_2024-07-31 12:52:32.csv', 'r')
# fo = open('result/scienceQA_sccot_deepseek_all_test_2024-06-04 02:15:43_fixed_ccot.csv', 'w')
reader = csv.reader(f)
...
for id in answer_list.keys():
    ...
    for i in range(5):
        ...
        label = label_dic[id]
        answer = answer_transform(answer, eval(options_dic[id]))

        if answer != '0' and answer != '1' and answer != '2' and answer != '3':
            continue
        else:
            # print(line_dic[id][i])
            # writer.writerow(line_dic[id][i])
            answer_dic[id][answer] += 1
            k = str([id, answer])
            if k in answer_dic_2.keys():
                answer_dic_2[k].append(line_dic[id][i])
            else:
                answer_dic_2[k] = [line_dic[id][i]]
...
for key in answer_dic.keys():
    ...
    for _ in answer_dic[key].keys():
        if answer_dic[key][_] > num:
            ...
            answer = _
    if answer == '':
        diff_list.append(line_dic[key][0])
        diff += 1
